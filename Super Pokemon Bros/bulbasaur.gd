...
func is_above():
	return GameState.player.position.y <= position.y + 10 and (((GameState.player.velocity.y > 0 || GameState.player.bounce)) || (GameState.player.jumptype == "spin" and GameState.player.velocity.y < 0) || (GameState.player.velocity.y >= 0 and !GameState.player.is_on_floor()))
